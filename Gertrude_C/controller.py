...
class PlayerController:
	...
	def play(
		...
		paint_budget = remaining_stamina - 20
		...
		## dont paint if enemy is close - fight instead
		if self.manhattan(my_loc, opponent.loc) <= 2:
			should_paint = False
		
		## dont paint if we want to double move
		if stamina > 70:
			should_paint = False
		if should_paint:
			paint_budget = min(remaining_stamina // 2, remaining_stamina - 20)
			paint_actions = self.choose_paints(board, player_parity, new_loc, paint_budget)
			# actions.extend(paint_actions)
			for p in paint_actions:
				if remaining_stamina >= 15:
					actions.append(p)
					remaining_stamina -= 15

		## move 2
		## enough for move and buffer
		if remaining_stamina >= 15:
				move_cost = 10

				## keep safety buffer
				if remaining_stamina >= move_cost + 10:
	...
	def choose_direction(self, board: Board, player_parity: int, from_loc: Optional[Location] = None, moves_done: int = 0) -> Optional[Direction]:
		...
		for d in Direction.cardinals():
			...
			if cell.owner_parity == opponent_parity:
				...
				## prioritze thicker layers
				score += 5 * layers

				## if thick tile then erase becomoes valuable
				if layers >= 2:
					erase_value = 25 + 10 * layers
	...
	def bfs_guidance_score(self, board: Board, player_parity: int, candidate_loc: Location) -> float:
		...
		## only take best target
		best_priority, best_target = max(targets, key = lambda x: x[0])
		...
		# 	score += delta * priority * 25
		# return score

		curr_dist = self.manhattan(player.loc, best_target)
		candidate_dist = self.manhattan(candidate_loc, best_target)

		return (curr_dist - candidate_dist) * best_priority * 30
	...
	def choose_paints(self, board: Board, player_parity: int, from_loc: Location, budget: int) -> List[Action.Paint]:
		...
		for d in Direction.cardinals():
			...
			if cell.owner_parity == -player_parity:
				# continue
				## allow painting if its a hill tile
				if cell.hill_id == 0:
					continue
